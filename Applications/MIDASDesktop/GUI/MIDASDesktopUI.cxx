...
MIDASDesktopUI::MIDASDesktopUI()
{
  ...
  // connect(dlg_createMidasResourceUI, SIGNAL(resourceCreated()), this,
  // SLOT(updateClientTreeView()));
  ...
  connect(dynamic_cast<GUIProgress *>(m_progress), SIGNAL(UpdateProgressMin(int) ), progressBar_current,
          SLOT(setMinimum(int) ) );
  connect(dynamic_cast<GUIProgress *>(m_progress), SIGNAL(UpdateProgressMax(int) ), progressBar_current,
          SLOT(setMaximum(int) ) );
  connect(dynamic_cast<GUIProgress *>(m_progress), SIGNAL(UpdateProgressValue(int) ), progressBar_current,
          SLOT(setValue(int) ) );
...
void MIDASDesktopUI::updateInfoPanel(const Midas3FolderTreeItem* folderTreeItem)
{
  ...
  if( folder->GetName() != "" )
    ...
    midasTreeItemInfoTable->setItem(i, 1,
  ...
  if( folder->GetDescription() != "" )
    {
    midasTreeItemInfoTable->setRowHeight(i, QTableWidgetDescriptionItem::rowHeight);
    midasTreeItemInfoTable->setItem(i, 0,
  ...
  int leftoverSpace = midasTreeItemInfoTable->width() - midasTreeItemInfoTable->columnWidth(0)
    - midasTreeItemInfoTable->columnWidth(1);
...
void MIDASDesktopUI::infoPanel(MidasBitstreamTreeItem* bitstreamTreeItem, bool edit)
{
  ...
  enableResourceEditing(false); // false for now (nothing to edit about a
                                // bitstream)
...
void MIDASDesktopUI::displayClientResourceContextMenu(QContextMenuEvent* e)
{
  ...
  if( DB_IS_MIDAS3 )
    ...
    if( index.isValid() )
      {
      MidasTreeItem* item =
        const_cast<MidasTreeItem *>(dynamic_cast<MidasTreeViewClient *>(treeViewClient)->getSelectedMidasTreeItem() );
...
void MIDASDesktopUI::addSubcommunity()
{
  ...
  dlg_createMidasResourceUI->SetParentResource(
...
void MIDASDesktopUI::addSubfolder()
{
  ...
  dlg_createMidasResourceUI->SetParentResource3(
    dynamic_cast<Midas3TreeViewClient *>(treeViewClient)->getSelectedMidasTreeItem() );
  dlg_createMidasResourceUI->exec();
}
...
void MIDASDesktopUI::openBitstream()
{
  ...
  if( DB_IS_MIDAS3 )
    ...
    path = dynamic_cast<Midas3BitstreamTreeItem *>(const_cast<Midas3TreeItem *>(
                                                     dynamic_cast<Midas3TreeViewClient *>(treeViewClient)->
